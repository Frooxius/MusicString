...
int main(int argc, char *argv[])
{
	...
	float *buf = new float[44100*60*8];

	cout << "Parsing... ";
	cout.flush();
	MStr2Audio gen = MStr2Audio(musstr, 44100);
	cout << "Generating... ";
	cout.flush();
	...
	try
	{
		num = gen.Generate(buf, 44100*60*8);
	}
	...
	header.bytes_per_second = 44100*sizeof(short);
	header.channels = 1;
	header.data[0] = 'd';
	header.data[1] = 'a';
	header.data[2] = 't';
  ...
  //short *pcmbuf = new short[44100*60*30];
	for(int i = 0; i < num; ++i)
  {
